...
const GAME_LENGTH = 5;
const SKILL_NAME = 'Mamamoo Trivia';
const FALLBACK_MESSAGE = `The ${SKILL_NAME} skill can't help you with that.  It can ask you questions about Mamamoo if you say start game. What can I help you with?`;
const FALLBACK_REPROMPT = 'What can I help you with?';
const WRONG_ANSWER_PRE = ['Whoops', 'Oh no', 'Darn', 'Aw man', 'Blast', 'Oh dear', 'Shucks', 'Yikes', 'Oof', 'Bummer', 'Rough time', 'There\'s always next time',];
const RIGHT_ANSWER_PRE = ['Booya', 'Nice', 'Good job', 'Hurray', 'Yay', 'Way to go', 'Awesome', 'Amazing', 'Whoo hoo', 'Well done', 'Alrighty', 'Congrats', 'Great job',];
var audioFile = '<audio src="https://feliciatrinh-mamamoo-trivia.s3-us-west-2.amazonaws.com/mamamoo-greeting.mp3" />';
...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (answerSlotValid
    ...
    currentScore += 1;
    injection = `<say-as interpret-as='interjection'>${RIGHT_ANSWER_PRE[getRandom(0, RIGHT_ANSWER_PRE.length - 1)]}! </say-as><break strength='strong'/>`;
    speechOutputAnalysis = requestAttributes.t('ANSWER_CORRECT_MESSAGE');
  } else {
    if (!userGaveUp) {
      injection = `<say-as interpret-as='interjection'>${WRONG_ANSWER_PRE[getRandom(0, WRONG_ANSWER_PRE.length - 1)]}! </say-as><break strength='strong'/>`;
...
const languageString = {
  en: {
    translation: {
      ...
      QUIT_MESSAGE: `Good bye.${audioFile}`,
      CANCEL_MESSAGE: `Ok, let's play again soon. ${audioFile}`,
      NO_MESSAGE: `Ok, we'll play another time. Goodbye!${audioFile}`,
      TRIVIA_UNHANDLED: 'Try saying a number between 1 and %s',
      HELP_UNHANDLED: 'Say yes to continue, or no to end the game.',
      START_UNHANDLED: 'Say start to start a new game.',
      NEW_GAME_MESSAGE: `${audioFile}Welcome to %s. `,
      ...
      GAME_OVER_MESSAGE: `You got %s out of %s questions correct. Thank you for playing!${audioFile}`,
